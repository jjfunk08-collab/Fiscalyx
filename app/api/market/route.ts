...
export async function GET(req: Request) {
  ...
  const finnhubQuotes: { inst: Instrument; fetchSymbol: string }[] = [];
  ...
  for (const sym of requested) {
    ...
    const isProxiedNonEquity =
      (inst.assetClass === "Index" || inst.assetClass === "Commodity") && !!inst.etf;

    if (r.coingeckoId) {
      cryptoIds.push(r.coingeckoId);
      cgInstById.set(r.coingeckoId, inst);
    } else if (useFinnhub && (inst.assetClass === "Equity" || isProxiedNonEquity)) {
      // Equities by ticker; indices/commodities by their ETF proxy ticker.
      finnhubQuotes.push({ inst, fetchSymbol: inst.etf ?? inst.symbol });
    } else if (
      useTwelveData &&
  ...
  const finnhubP =
    finnhubQuotes.length > 0
      ? Promise.allSettled(
          finnhubQuotes.map(async ({ inst, fetchSymbol }) => ({ inst, q: await fhQuote(fetchSymbol) }))
        )
      : Promise.resolve([]);
  ...
  for (let i = 0; i < finnhubResults.length; i++) {
    ...
    if (res.status === "fulfilled" && (num(res.value.q.c) ?? 0) > 0) {
      ...
      quotes.push({
        symbol: inst.symbol,
        name: inst.name,
        assetClass: inst.assetClass, // keep Index/Commodity grouping for proxies
