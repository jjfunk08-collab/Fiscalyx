...
import { isTwelveDataEnabled, tdTimeSeries } from "@/lib/twelvedata";
...
// Twelve Data interval + output size + cache TTL per timeframe.
function tdParams(tf: Timeframe): { interval: string; outputsize: number; ttl: number } {
  switch (tf) {
    case "1D":
      return { interval: "5min", outputsize: 500, ttl: 20_000 };
    case "1W":
      return { interval: "30min", outputsize: 500, ttl: 60_000 };
    case "1M":
      return { interval: "1day", outputsize: 40, ttl: 300_000 };
    case "1Y":
      return { interval: "1day", outputsize: 366, ttl: 300_000 };
    case "5Y":
      return { interval: "1week", outputsize: 300, ttl: 600_000 };
    case "MAX":
      return { interval: "1month", outputsize: 400, ttl: 600_000 };
  }
}
...
export async function GET(req: Request) {
  ...
  try {
    if (r.coingeckoId) {
      ...
      // Non-crypto: try providers in order, first non-empty wins.
      //   1) Twelve Data (keyed API, not IP-throttled)  — if enabled + mapped
      //   2) Finnhub candles (paid opt-in)              — equities only
      //   3) Yahoo chart()                              — keyless fallback

      // 1) Twelve Data
      if (candles.length === 0 && isTwelveDataEnabled() && r.twelvedata) {
        try {
          const { interval, outputsize, ttl } = tdParams(tf);
          const td = await tdTimeSeries(r.twelvedata, interval, outputsize, ttl);
          if (td.length > 0) {
            candles = td;
            source = "twelvedata";
          }
      ...
      // 2) Finnhub (paid candles)
      if (candles.length === 0 && finnhubCandlesEnabled() && r.instrument?.assetClass === "Equity") {
        try {
