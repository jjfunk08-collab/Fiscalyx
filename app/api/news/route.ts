...
function classify(text: string): NewsItem["category"] {
  ...
  if (
    /(fed|fomc|ecb|boe|boj|pboc|rba|central bank|rate decision|rate cut|rate hike|interest rate|basis points|powell|lagarde|bailey|ueda|monetary policy|quantitative (?:easing|tightening)|dot plot|fed funds|discount rate|reserve bank)/.test(
      t
    )
  )
    return "CentralBank";
...
export async function GET(req: Request) {
  ...
  if (isFinnhubEnabled()) {
    try {
      ...
      // When a symbol is active, pull its company news PLUS general market
      // news PLUS forex news. Company news alone is almost never tagged
      // Macro or CentralBank (Apple headlines don't mention CPI or the
      // Fed) — merging in broader feeds gives those tabs real headlines to
      // show instead of an empty pool. Forex news in particular tends to
      // carry more rate-decision / central-bank coverage than "general".
      const [companyRes, generalRes, forexRes] = await Promise.allSettled([
      ...
      const forexItems = forexRes.status === "fulfilled" ? fromFinnhub(forexRes.value) : [];
      const items = dedupe([...companyItems, ...generalItems, ...forexItems]);
