...
export default function TerminalHeader() {
  const { quoteMap, error, loaded, quotes } = useLive();
    ...
    <header className="border-b border-term-border bg-term-bg">
      ...
      <div className="flex items-center justify-between px-3 h-9">
        <div className="flex items-center gap-3">
          <span className="text-term-amber font-bold tracking-widest text-sm font-cond">
            ▮ FISCALYX
          </span>
          <span className="hidden sm:inline text-[10px] text-term-dim uppercase tracking-widest font-cond">
            Multi-Asset Market Terminal
          </span>
          {(() => {
            const state = !loaded
              ? { c: "text-term-amber", t: "CONNECTING" }
              : error && quotes.length === 0
              ? { c: "text-term-red", t: "ERROR" }
              : error
              ? { c: "text-term-amber", t: "RECONNECTING" }
              : { c: "text-term-green", t: "LIVE" };
            return (
              <span className={cn("flex items-center gap-1 text-[10px] font-cond", state.c)}>
                <Activity size={11} className={state.t === "LIVE" ? "animate-pulse" : "animate-blink"} />
                {state.t}
              </span>
            );
          })()}
