...
function CurveChart({ curve }: { curve: YieldPoint[] }) {
  const W = 720;
  const H = 240;
  const padL = 44;
  const padR = 16;
  ...
  return (
    <div className="p-3 border-b border-term-border">
      ...
      <p className="text-[9px] text-term-dim mt-2">
        Partial curve (13W · 5Y · 10Y · 30Y) from Yahoo CBOE yield indices. A full 1M–30Y curve
        requires a FRED / US Treasury data source not included in this key-free PoC.
      </p>
    </div>
  );
}
