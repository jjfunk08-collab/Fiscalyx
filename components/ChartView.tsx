...
export default function ChartView({ symbol }: { symbol: string }) {
  ...
  // Load (and, for intraday, poll) data on symbol / timeframe change.
  useEffect(() => {
    let alive = true;

    const applyData = (data: Candle[], fit: boolean) => {
    ...
    const load = async (fit: boolean) => {
      const res = await getHistory(symbol, tf);
      if (!alive) return;
      setLoading(false);
      setError(res.error);
      setNotFound(res.notFound && res.candles.length === 0);
      setCandles(res.candles);
      if (res.candles.length === 0) clearSeries();
      else applyData(res.candles, fit);
    };
    ...
    // Intraday timeframes refresh live every 10s (no re-fit so zoom is kept).
      ...
      <div className="panel-body relative">
        ...
        {!loading && notFound && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 z-10">
            <span className="text-term-red text-sm">Ticker Not Found</span>
            <span className="text-term-dim text-[10px]">No history for “{symbol}”.</span>
          </div>
        )}
        {!loading && !notFound && error && candles.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 z-10">
            <span className="text-term-red text-sm">No Data Available</span>
            <span className="text-term-dim text-[10px]">{error}</span>
          </div>
        )}
