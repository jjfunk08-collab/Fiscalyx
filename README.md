...
| Indices / FX / commodities (grid quotes) | Yahoo | Yahoo |
...
> **Provider notes:** Yahoo (`yahoo-finance2`) uses *unofficial* endpoints that rate-limit cloud/datacenter IPs (e.g. Vercel) — which is why **Twelve Data** (charts) and **FRED** (yields) exist as real keyed replacements. Twelve Data's free tier is ~800 calls/day; charts are cached and on-demand to stay under it. FRED requires an attribution line (shown in the yield panel) and is the official Fed source. Confirm each provider's commercial-use terms before charging for anything built on them.
