...
A Bloomberg Terminal–style, auto-updating market terminal built with **Next.js 14 (App Router) + TypeScript + Tailwind + Zustand + lightweight-charts**. It fetches **real market data with zero API keys** from two free public sources, entirely server-side: **Yahoo Finance** (via `yahoo-finance2`) for equities, indices, FX, commodities, treasury yields, fundamentals, options and news, and the **CoinGecko** public API for crypto. There is **no mock/fallback data** — panels show an explicit *Ticker Not Found* / *No Data Available* state when a source has nothing.
...
# set COINGECKO_API_KEY only if you start hitting CoinGecko 429s
```

Everything else needs no configuration. When a provider returns nothing (invalid ticker, rate limit, or outage), the affected panel renders an explicit **"Ticker Not Found"** / **"No Data Available"** state rather than fabricated numbers.
