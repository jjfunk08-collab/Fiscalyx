...
| Data | No key (default) | With `FINNHUB_API_KEY` |
| --- | --- | --- |
| US-equity quotes | Yahoo Finance | **Finnhub** (Yahoo fallback on miss) |
...
| Chart candles | Yahoo `chart()` | Yahoo (Finnhub candles are paid → `FINNHUB_ENABLE_CANDLES=true`) |
| Crypto | CoinGecko | CoinGecko |
| Indices / FX / commodities / treasuries | Yahoo | Yahoo (not on Finnhub free tier) |

It runs with an **empty `.env`** (Yahoo + CoinGecko, keyless). Dropping in a free Finnhub key routes US-equity quotes/news/fundamentals through Finnhub's licensed feed; upgrading the Finnhub plan lets you move charts over too. There is **no mock/fallback data** — panels show an explicit *Ticker Not Found* / *No Data Available* state when a source has nothing.

> **Data caveats:** Yahoo (`yahoo-finance2`) uses *unofficial* public endpoints — no key, but they can change/rate-limit without notice and are subject to Yahoo's terms (fine for a PoC, not a licensed feed). Finnhub's **free tier is ~60 req/min, blocks historical candles (HTTP 403), and doesn't cover indices/commodities/FX/treasuries** — which is exactly why those stay on Yahoo. Confirm Finnhub's commercial-use terms before charging for anything built on it.
...
### Optional: upgrade to Finnhub for equities

```bash
cp .env.example .env.local
# FINNHUB_API_KEY=xxxx        → routes US-equity quotes/news/fundamentals to Finnhub
# FINNHUB_ENABLE_CANDLES=true → only if you're on a PAID Finnhub plan (candles are 403 on free)
# COINGECKO_API_KEY=xxxx      → only if you hit CoinGecko 429s
```

Get a free Finnhub key at finnhub.io/register. With the key set, the status bar shows `finnhub · coingecko · yahoo` as the active mix.
