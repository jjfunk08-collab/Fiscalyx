...
// ---- Economic calendar (upcoming data-release dates) ----
// FRED's /releases/dates gives real scheduled release dates for economic
// data (CPI, GDP, employment, etc.). It's free with the same key. It does
// NOT carry forecast/consensus values (that's a paid-vendor feature), so we
// show the event + date honestly, labeled as release schedule.
...
// Marquee US macro releases worth surfacing (case-insensitive substring match).
const IMPORTANT = [
...
interface EconCacheEntry {
  ts: number;
  value: EconEvent[];
}
let econCache: EconCacheEntry | null = null;
const ECON_TTL = 6 * 60 * 60_000; // release schedule changes rarely → 6h

function fmtDate(d: Date): string {
  return d.toISOString().slice(0, 10);
}

/** Upcoming marquee US economic-data release dates (next ~45 days). Cached 6h. */
export async function fredReleaseDates(): Promise<EconEvent[]> {
  const key = fredKey();
  if (!key) return [];

  const now = Date.now();
  if (econCache && now - econCache.ts < ECON_TTL) return econCache.value;

  const today = new Date();
  const end = new Date(today.getTime() + 45 * 86_400_000);
  const url =
    `${BASE}/releases/dates?api_key=${encodeURIComponent(key)}&file_type=json` +
    `&realtime_start=${fmtDate(today)}&realtime_end=${fmtDate(end)}` +
    `&include_release_dates_with_no_data=true&sort_order=asc&limit=1000`;

  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) return [];
  const body: any = await res.json().catch(() => null);
  const rows: any[] = body?.release_dates ?? [];
  ...
  for (const r of rows) {
    ...
    if (!name || !date || date < todayStr) continue;
    const lower = name.toLowerCase();
    if (!IMPORTANT.some((k) => lower.includes(k))) continue;
    const dedupeKey = `${name}|${date}`;
  ...
  econCache = { ts: now, value: trimmed };
  return trimmed;
}
